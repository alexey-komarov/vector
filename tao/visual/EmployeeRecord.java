...
import tao.dialogs.*;
public class EmployeeRecord extends JPanel {
	...
	class AcSelect extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			try {
				DlgEmployers dlg = new DlgEmployers();
				dlg.setVisible(true);
				if (dlg.result) {
					try {
						ID = Integer.parseInt(dlg.getSelect(0));
						fDirectory.setText(dlg.getSelect("Family") + " " + dlg.getSelect("Name") + " " +dlg.getSelect("Patronymic"));
					} catch (Exception e) {
				...
				Dlg.error("Ошибка открытия справочника \"Сотрудники\"", e);
			}
		}
	}
