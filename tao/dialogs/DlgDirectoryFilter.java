...
public class DlgDirectoryFilter extends TaoDialog {
	public TaoDirectoryFilter result = new TaoDirectoryFilter();
	private DlgDirectoryFilter dlg;
	...
	private TaoDBComboBox cbValue1 = new TaoDBComboBox();
	private TaoDBComboBox cbValue2 = new TaoDBComboBox();


	public DlgDirectoryFilter(TaoDirectoryFilter aDirectoryFilter, String aCaption, String query) throws Exception {
		...
		rsDirectory = TaoGlobal.database.query.getNewResultSet(query);
		cbValue1.setSource(rsDirectory);
		cbValue2.setSource(rsDirectory);
		...
		cmbOper1.addItem("Код больше");
		cmbOper1.addItem("Код меньше");
		...
		pnl2.add(cbValue1, BorderLayout.CENTER);
		...
		pnl4.add(cbValue2, BorderLayout.CENTER);
		...
		if (aDirectoryFilter != null) {
			cmbOper1.setSelectedIndex(aDirectoryFilter.oper1);
			cmbOper2.setSelectedIndex(aDirectoryFilter.oper2);
			cmbOper3.setSelectedIndex(aDirectoryFilter.oper3);
			cbValue1.setValue(aDirectoryFilter.value1);
			cbValue2.setValue(aDirectoryFilter.value2);
		}
		updateControls();
	}
	...
	private void updateControls() {
		if (cmbOper1.getSelectedIndex() == 0) {
			cbValue1.setEnabled(false);
			cbValue2.setEnabled(false);
			cmbOper2.setEnabled(false);
			cmbOper3.setEnabled(false);
		} else {
			cmbOper2.setEnabled(true);
			cbValue1.setEnabled(true);
	...
	class AcOk extends AbstractAction {
		...
		public void actionPerformed(ActionEvent event) {
			...
			dlg.result.value1 = Integer.parseInt(cbValue1.getValue());
			dlg.result.oper2 = cmbOper2.getSelectedIndex();
			dlg.result.oper3 = cmbOper3.getSelectedIndex();
			dlg.result.value2 = Integer.parseInt(cbValue1.getValue());
