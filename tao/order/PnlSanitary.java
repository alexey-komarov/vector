...
public class PnlSanitary extends JPanel {
	...
	public PnlSanitary() {
		...
		try {
			...
			Dlg.error("Ошибка чтения списка сантехники", e);
		}
	...
	class AcAdd extends AbstractAction {
		AcAdd() {
			...
			putValue(Action.SHORT_DESCRIPTION, "Добавить позицию сантехники в заказ");
	...
	class AcEdit extends AbstractAction {
		AcEdit() {
			...
			putValue(Action.SHORT_DESCRIPTION, "Редактировать позицию сантехники");
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (dlg.result) {
				...
				try {
					...
					Dlg.error("Ошибка редактирования позиции сантехники в заказе", e);
				}
